 ...
 * of WebDriver: 1) Opens all the browsers with the url specified in JITSI_URL. 2) Do the following
 * every 1 second for 1 minute: a) Executes the JavaScript on all browsers given via
 * getConnectionScript() which returns iceConnectionState. b) Executes the JavaScript on all
 * browsers given via getNumberOfParticipantsScript() which returns number of participants. b)
 * Checks whether all the browsers have returned either 'completed' or 'connected' for
 * iceConnectionState and correct number of participants. 4) The test is considered as successful if
 * all the browsers either returns 'completed' or 'connected' with correct number of participants
 * within 1 minute. 5) A successful test returns a boolean 'true' while the unsuccessful test
 * returns a boolean 'false'
 */
public class JitsiTest extends KiteTest {

  private static final Logger logger = Logger.getLogger(JitsiTest.class.getName());
  ...
  private static final String JITSI_URL = "https://meet.jit.si/";
  ...
  /**
   * Returns the test getConnectionScript to retrieve APP.conference.getConnectionState(). If it
   * doesn't exist then the method returns 'unknown'.
   *
  ...
  /**
   * Returns the test getNumberOfParticipantsScript to retrieve APP.conference.membersCount. If it
   * doesn't exist then the method returns 0.
   *
  ...
  /**
   * Checks whether all of the iceConnectionState strings match at least one entry from the
   * expectedResultMap. Checks whether all of the participant numbers match the tuple size.
   *
    ...
    for (int i = 0; i < TIMEOUT; i += INTERVAL) {
      ...
      for (WebDriver webDriver : this.getWebDriverList()) {
        String connectionState =
            (String) ((JavascriptExecutor) webDriver).executeScript(this.getConnectionScript());
        long numberOfParticipants =
            (Long)
                ((JavascriptExecutor) webDriver)
                    .executeScript(this.getNumberOfParticipantsScript());
