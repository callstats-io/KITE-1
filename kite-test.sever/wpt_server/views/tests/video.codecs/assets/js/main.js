...
function findVideoMline(line) {
  return line.startsWith('m=video');
}

function modifyVideoMline (line, codecCode){
  if (codecCode === null) {
    trace('Restricting codec not found');
    // skipping m=video line to corrupt the video
    return '';
  } else {
    trace('Restricting codec code found: '+ codecCode);
    // keeping only the required codec
    trace('Changing this: '+ line);
    var words = line.split(' ');
    var newLine = words[0] + ' ' + words[1] + ' ' + words[2] + ' ' + codecCode;
    trace('to this: '+ newLine);
    return newLine;
  }
}


function keepOnlyOneCodec(desc, codec){
  var sdp = desc.sdp;
  var inMlineSection = false;
  trace("SDP before modifications:");
  trace(sdp);
  var lines = sdp.split('\r\n');
  var codecCode = null;
  ...
  lines.forEach(function(line) {
    if (line.startsWith('a=rtpmap')){
      if (line.includes(codec)){
        codecCode = line.split(':')[1];
        codecCode = codecCode.split(' ')[0];
      }
    }
  });

  var mLineIndex = lines.findIndex(findVideoMline);
  var newLines = [];
  lines[mLineIndex] = modifyVideoMline(lines[mLineIndex], codecCode);
  if (lines[mLineIndex] !== '') {
    lines.forEach(function(line){
      if (line.startsWith('m=video')){
        inMlineSection = true;
        trace('found video line at' + lines.indexOf(line));
      }
      if (line.startsWith('m=audio')){
        inMlineSection = false;
        trace('found audio line at' + lines.indexOf(line));
      }
      if (inMlineSection){
        if (line.startsWith('a=rtpmap') ||
              line.startsWith('a=rtcp-fb') ||
                line.startsWith('a=fmtp')){
          if (line.split(':')[1].split(' ')[0] !== codecCode){
            trace('removing stuff ->' + line);
          } else {
            newLines.push(line);
          }
        } else {
  ...
  desc.sdp = newLines.join('\r\n');
  trace("SDP after modifications:");
  trace(desc.sdp);
}
