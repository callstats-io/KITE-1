...
package org.webrtc.kite.apprtc.bitrate;
...
/**
 * ARBitRateTest implementation of KiteTest.
 * <p>
 ...
 * stashed earlier.</li>
 * </ul>
 * <li>4) The test is considered as successful if the bitrate of the received audio is equal to the one in the option</li>
 * <li>5) A successful test returns a boolean 'true' while the unsuccessful test returns a boolean
 * 'false'.</li>
 * </ul>
 * </p>
 */
public class ARBitRateTest extends KiteTest {

  private final static Logger logger = Logger.getLogger(ARBitRateTest.class.getName());
  ...
  private class Tester implements Callable<JsonObject> {
    ...
    public JsonObject call() throws Exception {
      ...
      if (everythingOK){
        double bitrate = Utility.getBitrate(webDriver, "audio", false);

        everythingOK = (valueDouble / 1.050 < bitrate && bitrate < valueDouble / 0.95);
        if (!everythingOK){
          message = "Bitrate was: " + bitrate + " while expected bitrate was: " + valueDouble;
