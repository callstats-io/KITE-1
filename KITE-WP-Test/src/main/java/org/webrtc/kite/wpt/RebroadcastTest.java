...
/**
 * RebroadcastTest implementation of KiteTest.
 *
 ...
 *   <li>2) Get local media stream.
 *   <li>3) Waits for local video to be displayed on page and verifies if they are actually being
 *       displayed
 *   <li>4) Send media stream to 2nd peer connection, this will create another peer connection and send to the third one.
 *   <li>5) The test is considered as successful if all the videos are displayed correctly.
 * </ul>
 */
public class RebroadcastTest extends KiteTest {

  private static final Logger logger = Logger.getLogger(RebroadcastTest.class.getName());
  ...
  private class Tester implements Callable<JsonObject> {
    ...
    public JsonObject call() throws Exception {
      ...
      try {
        ...
        if (!everythingOK) {
          logger.error("Intermediary peer connection couldn't display video");
          res.add("result", false);
          stats.add("log", getLog(this.webDriver));
          res.add("stats", stats);
          return res.build();
        } else {
          button = this.webDriver.findElement(By.id("rebroadcastButton"));
          button.click();
          everythingOK = checkVideoDisplay(this.webDriver, 2, TIMEOUT, INTERVAL);
        }

        if (!everythingOK) {
          logger.error("Third peer connection couldn't display rebroadcasted video");
          res.add("result", false);
          stats.add("log", getLog(this.webDriver));
          res.add("stats", stats);
          return res.build();
