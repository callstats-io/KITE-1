...
/**
 * MediaRecorderAPITest implementation of KiteTest.
 *
 ...
 *   <li>2) Wait for local media stream to play and click on record button.
 *   <li>3) Wait for 5s then click on stop recording button.
 *   <li>4) Play recorded video.
 *   <li>5) Waits for recorded video to be displayed on page and verifies if they are actually being
 *       displayed
 *   <li>6) The test is considered as successful if all the videos are displayed correctly.
 * </ul>
 */
public class MediaRecorderAPITest extends KiteTest {

  private static final Logger logger = Logger.getLogger(MediaRecorderAPITest.class.getName());
  ...
  private class Tester implements Callable<JsonObject> {
    ...
    public JsonObject call() throws Exception {
      ...
      try {
        ...
        wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.id("gum")));
        everythingOK = checkVideoDisplay(this.webDriver, 0, TIMEOUT, INTERVAL);

        WebElement button = this.webDriver.findElement(By.id("record"));
        button.click();
        ...
        if (!everythingOK) {
          ...
          Thread.sleep(5000);
          button.click();
          button = this.webDriver.findElement(By.id("play"));
          button.click();
          Utility.playVideo(this.webDriver, "recorded");
          Utility.checkVideoDisplay(this.webDriver, 1, TIMEOUT, INTERVAL);
        }

        if (!everythingOK) {
          logger.error("Recorded video didn't play correctly");
          res.add("result", false);
          stats.add("log", getLog(this.webDriver));
          res.add("stats", stats);
          return res.build();
